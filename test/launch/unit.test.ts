...
describe('#Launch', () => {
  // Skip all tests if local dev environment is not available
  let itIfEnv: typeof it = it.skip // Default to skip, enabled in beforeAll if env available
  ...
  beforeAll(async () => {
    ...
    itIfEnv = it // Enable tests
    publicClient = env.publicClient
    walletClient = env.walletClient

    // Configure GraphQL client to use the local indexer first
    Client.updateUrl(LOCAL_GRAPHQL_URL)

    // Get FloorFactory from the indexer (no need for .env file)
  ...
  describe('Schema Validation', () => {
    itIfEnv('should validate a correct floor config', () => {
    ...
    itIfEnv('should reject floor config with invalid buyFeeBps', () => {
      const config = createTestFloorConfig(ANVIL_ADDRESSES.ADMIN, ANVIL_ADDRESSES.MANAGER, {
        buyFeeBps: 15000,
      })
    ...
    itIfEnv('should reject floor config with empty segments', () => {
      const config = {
    ...
    itIfEnv('should reject floor config with invalid numberOfSteps', () => {
    ...
    itIfEnv('should validate a correct treasury config', () => {
    ...
    itIfEnv('should reject treasury config with empty recipients', () => {
      const config = {
    ...
    itIfEnv('should validate a correct credit facility config', () => {
    ...
    itIfEnv('should reject credit facility config with LTV > 10000', () => {
    ...
    itIfEnv('should reject credit facility config with maxLeverage > 255', () => {
      const config = { loanToValueRatio: 9900, maxLeverage: 300, borrowingFeeRate: 0 }
      expect(() => launch.validateCreditFacilityConfig(config)).toThrow()
    })

    itIfEnv('should validate a complete launch config', () => {
  ...
  describe('Encoding', () => {
    itIfEnv('should encode floor config correctly', () => {
      const config = createTestFloorConfig(ANVIL_ADDRESSES.ADMIN, ANVIL_ADDRESSES.MANAGER)
    ...
    itIfEnv('should encode authorizer config correctly', () => {
    ...
    itIfEnv('should encode treasury config correctly', () => {
      const config = createTestTreasuryConfig(ANVIL_ADDRESSES.ADMIN)
    ...
    itIfEnv('should encode credit facility config correctly', () => {
    ...
    itIfEnv('should encode presale config correctly', () => {
    ...
    itIfEnv('should reject segment with initialPrice exceeding 72-bit max', () => {
      const config = createTestFloorConfig(ANVIL_ADDRESSES.ADMIN, ANVIL_ADDRESSES.MANAGER, {
        segments: [
          {
    ...
    itIfEnv('should reject segment with numberOfSteps = 0', () => {
      const segments = createTestSegments()
      const config = createTestFloorConfig(ANVIL_ADDRESSES.ADMIN, ANVIL_ADDRESSES.MANAGER, {
        segments: [{ ...segments[0], numberOfSteps: 0 }],
      })
  ...
  describe('RPC Reads', () => {
    itIfEnv('should connect to local anvil and get block number', async () => {
    ...
    itIfEnv('should read floor ID counter from FloorFactory', async () => {
    ...
    itIfEnv('should read module factory address from FloorFactory', async () => {
    ...
    itIfEnv('should get deployer account balance', async () => {
  ...
  describe('GraphQL Indexer', () => {
    itIfEnv('should fetch markets from indexer', async () => {
    ...
    itIfEnv('should query the GraphQL endpoint directly', async () => {
  ...
  describe('E2E Create Flow', () => {
    itIfEnv('should create a new floor market with freshly deployed tokens', async () => {
